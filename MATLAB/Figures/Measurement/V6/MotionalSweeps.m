...
%Setup sweep matrix for all rabi frequencies
SweepRatesRabi = repmat(SweepRates, 1, length(RabiFreqs)); 
%Start with fidelity 1
Prob = 1;
%Go through each detuning, calculating error
for i = 1:length(Detunings)
    prob = Prob5_v2(G.Linewidth, RabiFreqs, SweepRatesRabi, Detunings(i), G.Fidelity, Orders(i));
    Prob = Prob.*(1-prob);
    if DriveTwice(i) == "yes"
        Prob = Prob.*(1-prob);
    end
