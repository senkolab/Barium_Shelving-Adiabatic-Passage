function [Line, ProbIdeal, TotalTime] = ...
...
for i = 1:size(Measurement, 1)
    ...
    if Type == "Shelve"
        ...
        if any(isnan(MeasurementStep))
            fprintf("Step number %i %s for d = %i failed. \nOne of the states not specified. \nThe current ground state status is\n", i, Type, Level);
            disp(LevelsG);
            fprintf("The current shelving state status is\n")
            disp(LevelsP);
            continue;
        end
        ...
        if i == 1
            TotalTime = TotalTimeTransfer;
        else
            TotalTime = TotalTime + TotalTimeTransfer;
        end
        Prob = Prob.*ProbTransfer;
    ...
    elseif Type == "Fluoresce"
        ...
        fprintf("Step number %i %s for d = %i failed. \nYou've input a measurement step name wrong. \nMake sure all steps are called ""Shelve"", ""Deshelve"", ""Hide"", or ""Fluoresce""\n", i, Type, Level);
    end
    disp("     LevelsG    LevelsP");
    disp([LevelsG LevelsP]);
    fprintf("\n");
end
%Optimize over Rabi frequencies
[ProbIdeal, index] = max(Prob);
