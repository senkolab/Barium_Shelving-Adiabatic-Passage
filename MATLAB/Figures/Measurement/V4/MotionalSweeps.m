function [Prob] = MotionalSweeps(DetuningsMatrix, SweepRates, RabiFreqs, indexx)
...
for i = 1:length(Detunings)
    prob = Prob5_v2(G.Linewidth, RabiFreqs(indexx), SweepRates(indexx), Detunings(i), G.Fidelity, Orders(i));
    Prob = Prob.*(1-prob);
    if DriveTwice(i) == "yes"
        Prob = Prob.*(1-prob);
    end
